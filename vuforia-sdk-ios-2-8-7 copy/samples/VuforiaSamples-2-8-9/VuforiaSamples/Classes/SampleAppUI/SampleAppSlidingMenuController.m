...
- (id)initWithRootViewController:(UIViewController*) controller {
    if ((self = [super init])) {
        ...
        self.menuViewController = [[SampleAppLeftMenuViewController alloc] init];
...
- (void)viewDidLoad {
    ...
    if (! ignoreDoubleTap) {
        doubleTap = [[UITapGestureRecognizer alloc] initWithTarget: self action:@selector(doubleTapGestureAction:)];
...
- (void)panGestureAction:(UIPanGestureRecognizer*)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        NSMutableArray *animationValues = [[NSMutableArray alloc] initWithCapacity:2];
