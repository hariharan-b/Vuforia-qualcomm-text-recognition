...
- (void)renderFrameQCAR
{
    ...
    if (NbWordsFound > 0) {
    	...
    	for (int i = 0 ; i < NbWordsFound ; i++) {
            ...
            NSString *wordsFound = temp;
            NSLog(@":::::%@",wordsFound);

           // [_arrayWords addObject:wordsFound];
           // [self.arrayWords addObject:[NSString stringWithFormat:@"%@",wordsFound]];
          //  NSLog(@"string founded are %@", _arrayWords);
    	}
...
- (void)initShaders
{
    lineShaderProgramID = [SampleApplicationShaderUtils createProgramWithVertexShaderFileName:@"Line.vertsh" fragmentShaderFileName:@"Line.fragsh"];
